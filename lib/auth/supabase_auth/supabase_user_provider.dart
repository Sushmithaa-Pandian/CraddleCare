...
class GluaHealthAppTemplateSupabaseUser extends BaseAuthUser {
  GluaHealthAppTemplateSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> gluaHealthAppTemplateSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GluaHealthAppTemplateSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
